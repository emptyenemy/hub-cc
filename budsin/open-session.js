// budsin/open-session.js
//
// Открывает видимый Chromium с ПЕРСОНАЛЬНЫМ ПРОФИЛЕМ аккаунта (полный профиль
// на диск: история, куки, localStorage, сессии GitHub + budsin).
//
// Панель — New API `v1.0.0-rc.39` (замер `/api/status` 21.09), механизм тот же, что у
// kktoken/nova: реф-код в localStorage, GitHub-OAuth, кука ЛК, разбор ошибок сайта.
// Отличаются только адреса и реф-код.
//
// Чем budsin отличается (замер `/api/status` 21.09):
//   • вход через GitHub-OAuth есть (`github_oauth: true`) — путь тот же;
//   • ✅ капчи НЕТ (`turnstile_check: false`) и почта не подтверждается
//     (`email_verification: false`) — автореге площадка не мешает. ⚡-автозаведения
//     у вкладки всё равно нет (не делали), сценарий ниже — ручной;
//   • ⚠️ пароль-регистрация выключена (`password_register_enabled: false`) — вход
//     только через GitHub.
//
// Сценарий:
//   1. В дашборде добавляешь аккаунт (email, ключ можно оставить пустым), жмёшь
//      🌐 «Открыть браузер».
//   2. Открывается Chromium с профилем budsin/profiles/<label>/ (на аккаунт).
//   3. Ключа у аккаунта ещё нет → открывается РЕГИСТРАЦИЯ по рефке владельца.
//      Ключ уже вписан → открывается страница баланса (wallet).
//   4. В консоли BudsAI возьми API-ключ и вставь его в аккаунт кнопкой 🔑
//      на дашборде (или впиши сразу при добавлении).
//   5. Профиль сохраняется автоматически — при следующих открытиях GitHub и
//      BudsAI уже залогинены.
//
// Использование:
//   node budsin/open-session.js <label> [register|console|auto]
//     label — имя профиля (папка budsin/profiles/<label>/)
...
// Рефка владельца: аккаунт без ключа регистрируем ТОЛЬКО по ней (код `kSyu`).
// ⚠️ Обещанный площадкой подарок («$15» из рекламы) ключом не подтверждён: на
// разобранном ключе остаток был **$0.10** (замер 21.09, разбор — в вики).
...
const REGISTER_URL = require("../routing/lib/ref-codes.js").url("budsin");
// Ключ уже вписан → сразу баланс, а не логин.
const CONSOLE_URL = 'https://apichat.budsin.dev/wallet';
// Корень нужен для прогрева перед регистрацией (см. openRegisterViaRef).
const ROOT_URL = 'https://apichat.budsin.dev/';
...
const ghCapture = require('../routing/lib/gh-live-capture.js').makeCapture({
  label,
  moduleDir: __dirname,
  poolFile: path.join(__dirname, '..', 'routing', 'budsin-sessions.json'),
});
...
//   share-код друга      → аккаунт budsin уже создан, GitHub/budsin сразу залогинены;
//   seed:'github'        → только GitHub-куки, аккаунта budsin ещё НЕТ (см. seededGithub).
...
// (поймано на ar-аккаунтах 2026-08-17, у gorouter/tabi/budsin тот же NewAPI-фронт).
...
// Ответы budsin (NewAPI) на неудачный GitHub-вход. Раньше скрипт знал только
...
const SITE_ERRORS = [
  {
    ...
    msg: '❌ budsin закрыл регистрацию новых аккаунтов (ответ сайта) — этот аккаунт создать нельзя.',
  },
  {
    code: 'git_token',
...
// GitHub-вход прошёл и мы внутри budsin (консоль/дашборд). Тогда профиль уже
...
async function main() {
  ...
  try {
    if (appliedSession && !seededGithub) {
      ...
      console.log('✅ Импортированная сессия применена (GitHub/budsin уже залогинены).');
    ...
    if (wantRegister) {
      ...
      console.log('   затем возьми ключ в консоли BudsAI и вставь его кнопкой 🔑 в дашборде.');

      const res = await waitForLogin(page, context);
      if (!res.ok) {
        if (res.err && res.err.code === 'no_register') {
          console.error('❌ Регистрация на budsin закрыта администратором — новый аккаунт не создать.');
    ...
    if (!fresh) {
      await reportRender(page);
      console.log('✅ Профиль восстановлен (GitHub/budsin уже залогинены, если заходил раньше).');
      console.log('   Браузер открыт — закрой когда закончишь (Ctrl+C).');
      await ghCapture.holdOpen(context); // держим открытым, закрытие — вручную
      return;
    }
    ...
    if (!loggedInEarly) console.log('   затем возьми ключ в консоли BudsAI и вставь его кнопкой 🔑 в дашборде.');
